...
class buttons():
    def __init__(self,
        ...
        self.help_button = self.helpButton(
            main_window = self.main_window,
            button_assets = self.button_assets,
            layout = self.layout
        )
...
label : QLabel = None
class Functions:
    def help(main_window: QMainWindow | QWidget):
        message = "Howdy!\n1. Launch into a Jam Stage lobby\n2. Press the start button and click into the game.\n\nBoom| Enjoy!"
    ...
    def start(
        ...
        if is_pressed is False:
            log.info("Launching jam fortnite thread.")
            ...
            # Start the thread.
            thread = threading.Thread(target = pad.run_jam, args = (stop_event,))
            thread.start()
